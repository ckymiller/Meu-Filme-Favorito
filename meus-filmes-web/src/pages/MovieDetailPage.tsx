  ...
  return (
    <div style={{ minHeight: "100dvh", background: "var(--bg)", display: "flex", flexDirection: "column" }}>
      ...
      <header className="topbar">
        <button className="icon-btn" onClick={() => navigate(-1)} style={{ fontSize: 20 }}>
          ←
        </button>
        <span className="topbar-title" style={{ fontSize: 15, fontWeight: 600 }}>
          Detalhe
        </span>
        <div style={{ width: 36 }} />
      </header>
        ...
        <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 20, maxWidth: 720, margin: "0 auto" }}>
          ...
          <div style={{ display: "flex", gap: 16 }}>
            <div
              ...
              style={{ width: 110, height: 165, flexShrink: 0, fontSize: 36, borderRadius: 10, border: "1px solid var(--card-border)" }}
            >
              {movie.posterUrl
                ? <img src={movie.posterUrl} alt={movie.titlePtBr} />
                : "🎬"}
            </div>
            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              {movie.originalTitle !== movie.titlePtBr && (
                <p style={{ fontSize: 14, color: "var(--muted-fg)", fontStyle: "italic" }}>
                  {movie.originalTitle}
                </p>
              )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            <span className="section-label">Status atual</span>
            <div
              style={{
                background: "var(--card)",
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                {otherStatuses.map((s) => (
                  <button
                    ...
                    onClick={() => updateStatus(movie.id, s)}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {movie.tmdbId == null ? (
              <p style={{ fontSize: 14, color: "var(--muted-fg)", lineHeight: 1.6 }}>
                Filme adicionado manualmente — sinopse indisponível.
              </p>
            ) : isFetching ? (
              ...
              <>
                {tagline && (
                  <p style={{ fontSize: 14, color: "var(--muted-fg)", fontStyle: "italic", lineHeight: 1.5 }}>
                    "{tagline}"
                  </p>
                )}
